...
class Network:
    ...
    # Establishes a connection to the server, or thread id 0
    ...
    # Sends data that is either a string or a an object needing to be pickled
...
class Enemy:
    ...
    # Sends coordinates of the enemy
...
class Missile:
    ...
    # Changes state so that the missile starts to get shot
    def ChangeState(self):
        self.state = 'shot'

    # Takes the X coordinate of the missile about to be shot so that it can blit it
    ...
    # Returns coordinates of the missile
    def getCoords(self):
        if self.currentx == []:
            pass
        else:
        ...
        for missile in Player1Obj.missiles:
            ...
            if a == None:
                ...
                if missile.state == 'ready':
                    break
                if Bob.y - 20 <= a[1] <= Bob.y + 20 and Bob.x - 30 < a[0] <= Bob.x + 80:
                    print('Bob')
                    missile.state = 'ready'
                    missile.y = 700
                    Bob.x = 0
                    Bob.y = 0
                elif Martin.y - 20 <= a[1] <= Martin.y + 20 and Martin.x - 30 < a[0] <= Martin.x + 80:
                    print('Martin')
                    missile.state = 'ready'
                    hit = 1
                    missile.y = 700
...
# ================================================# Server connection #================================================#
        ...
        response = Server.send([[Player1Obj.x], Bob.SendCoordinates(), hit], pick=True)
        print(response)
        Bob.x = response[1][0]
        Bob.y = response[1][1]
